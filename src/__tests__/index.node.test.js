...
describe('local-persist', () => {
  ...
  typesAndValues.forEach(({ type }) => {
    it(`should remove "${type}" type`, () => {
      ...
      expect(localPersist.get(type)).toBeUndefined()
    })
  })
  ...
  it(`should not remove a non-existent key`, () => {
    ...
    expect(localPersist.remove(key)).toBeFalsy()
  })
  ...
  it(`should clear all items`, () => {
    ...
    expect(localPersist.get(first.type)).toBeUndefined()
    expect(localPersist.get(second.type)).toBeUndefined()
  })
})
